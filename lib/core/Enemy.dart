...
abstract class Enemy extends AnimatedGameObject with ObjectCollision {
  double life;
  double _maxlife;
  final double speed;
  //millesegundos
  ...
  setInitPosition(Rect position) {
    if (!_isSetPosition) {
      _isSetPosition = true;
      this.position = Rect.fromLTWH(position.left, position.top,
  ...
  void moveToHero(Player player, Function() attack) {
    ...
    if (player != null) {
      ...
      Rect fieldOfVision = Rect.fromLTWH(
          _currentPosition.left - (visionWidth / 2),
          _currentPosition.top - (visionHeight / 2),
          visionWidth,
          visionHeight);
      ...
      if (fieldOfVision.overlaps(player.position)) {
        double translateX =
            _currentPosition.center.dx > leftPlayer ? (-1 * speed) : speed;
        double translateY =
            _currentPosition.center.dy > topPlayer ? (-1 * speed) : speed;
  ...
  void _drawLife(Canvas canvas) {
    ...
    canvas.drawLine(
        Offset(_currentPosition.left, _currentPosition.top - 4),
    ...
    canvas.drawLine(
        Offset(_currentPosition.left, _currentPosition.top - 4),
        Offset(
            _currentPosition.left + currentBarLife, _currentPosition.top - 4),
        Paint()
